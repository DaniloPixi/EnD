...
export const store = createStore<State>({
  ...
  mutations: {
    setMemos(state, memos: any[]) {
      state.memos = memos;
    },
    addMemo(state, memo: any) {
      state.memos.unshift(memo);
    },
    deleteMemo(state, memoId: string) {
      state.memos = state.memos.filter(memo => memo.id !== memoId);
    },
    editMemo(state, updatedMemo: { id: string; content: string; tags: string[]; imageUrl?: string | null }) {
  ...
  actions: {
    ...
    async addMemo({ commit }, memoData: any) {
      const db = getFirestore();
      const memosCollection = collection(db, 'memos');
    ...
    async editMemo({ commit }, updatedMemo: { id: string; content: string; tags: string[]; imageUrl?: string | null }) {
      const db = getFirestore();
      ...
      try {
        ...
        // It's crucial to check for 'undefined' to distinguish from 'null' (which means delete the image)
        if (updatedMemo.imageUrl !== undefined) {
          updateData.imageUrl = updatedMemo.imageUrl;
        }
